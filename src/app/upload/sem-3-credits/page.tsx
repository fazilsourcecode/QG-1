...
// Define the question structure for SEM 3-Credits
const questionGroups = {
  ...
  // Part B: 16-mark grouped questions (choose one from A, B, C for each main number)
  ...
  "15A": ["15a(i)", "15a(ii)", "15a(iii)"],
  "15B": ["15b(i)", "15b(ii)", "15b(iii)"],
  "15C": ["15c(i)", "15c(ii)", "15c(iii)"],
  Q15_Total: ["15A", "15B", "15C"],
}

// Maximum marks for each question and subpart for SEM 3-Credits
const maxMarks = {
  // Individual questions (2 marks each)
  "1": 2, "2": 2, "3": 2, "4": 2, "5": 2,
  "6": 2, "7": 2, "8": 2, "9": 2, "10": 2,
  PartA_Total: 20, // Q1-Q10 total

  // Q11 subparts (each subpart can have max 16, but only one group should be active)
  "11a(i)": 16, "11a(ii)": 16, "11a(iii)": 16,
  "11b(i)": 16, "11b(ii)": 16, "11b(iii)": 16,
  "11c(i)": 16, "11c(ii)": 16, "11c(iii)": 16,
  Q11_Total: 16,

  "12a(i)": 16, "12a(ii)": 16, "12a(iii)": 16,
  "12b(i)": 16, "12b(ii)": 16, "12b(iii)": 16,
  "12c(i)": 16, "12c(ii)": 16, "12c(iii)": 16,
  Q12_Total: 16,

  "13a(i)": 16, "13a(ii)": 16, "13a(iii)": 16,
  "13b(i)": 16, "13b(ii)": 16, "13b(iii)": 16,
  "13c(i)": 16, "13c(ii)": 16, "13c(iii)": 16,
  Q13_Total: 16,

  "14a(i)": 16, "14a(ii)": 16, "14a(iii)": 16,
  "14b(i)": 16, "14b(ii)": 16, "14b(iii)": 16,
  "14c(i)": 16, "14c(ii)": 16, "14c(iii)": 16,
  Q14_Total: 16,

  "15a(i)": 16, "15a(ii)": 16, "15a(iii)": 16,
  "15b(i)": 16, "15b(ii)": 16, "15b(iii)": 16,
  "15c(i)": 16, "15c(ii)": 16, "15c(iii)": 16,
  Q15_Total: 16,

  Grand_Total: 100,
}

// Group questions by their main number for SEM 3-Credits
...
// Define the Excel column structure for SEM 3-Credits
const excelColumns = [
  "Name", "RRN", "Course",
...
export default function UploadSem3CreditsPage() {
  ...
  const generateExcelData = useCallback(
    (data: ExtractHandwrittenMarksOutput[]) => {
      return data.map((item) => {
        ...
        Object.keys(questionsByMainNumber).forEach(mainNumKey => { // Q11-Q15 groups for SEM
  ...
  const handleDownloadExcel = useCallback(() => {
    if (excelFileBlob) {
      FileSaver.saveAs(excelFileBlob, `sem_3_credits_marksheet_data_${new Date().toISOString().slice(0, 10)}.xlsx`)
  ...
  const handleViewDetails = (data: ExtractHandwrittenMarksOutput) => {
    ...
    setTimeout(() => scrollToElement("marksheet-details-sem3"), 100)
  }

  const handleViewValidationIssues = (status: ProcessingStatus) => {
    setSelectedStatus(status)
    setTimeout(() => scrollToElement("validation-issues-sem3"), 100)
  }
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      <Card className="w-full max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            SEM Marksheet (3 Credits) - Batch Processing
          </CardTitle>
          <CardDescription>
            Upload 3-credit SEM marksheet images to extract and compile data.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="single" onValueChange={(v) => setUploadMode(v as "single" | "multiple" | "zip")} className="mb-6">
            ...
            <TabsContent value="single"><p className="text-sm text-muted-foreground mb-4">Upload a single 3-credit SEM marksheet image.</p></TabsContent>
            <TabsContent value="multiple"><p className="text-sm text-muted-foreground mb-4">Upload multiple 3-credit SEM marksheet images.</p></TabsContent>
            <TabsContent value="zip"><p className="text-sm text-muted-foreground mb-4">Upload a ZIP file containing 3-credit SEM marksheet images.</p></TabsContent>
          </Tabs>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField control={form.control} name="files" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="space-y-4">
                        <Label htmlFor="upload-files-sem3" className={cn("flex flex-col items-center justify-center h-40 border-2 border-dashed border-input bg-background px-3 py-2 cursor-pointer rounded-md transition-colors", isProcessing ? "cursor-wait" : "hover:bg-accent/50", selectedFiles && selectedFiles.length > 0 ? "border-primary bg-primary/5" : "")}>
                          ...
                          <Input id="upload-files-sem3" ref={fileInputRef} type="file" accept={fileAccept} multiple={uploadMode !== "single"} disabled={isProcessing} onChange={(e) => { const files = e.target.files; field.onChange(files); setSelectedFiles(files); }} className="hidden"/>
                        </Label>
                    ...
                    <FormDescription>
                      {uploadMode === "single" ? "Upload the 3-credit SEM marksheet image." : uploadMode === "zip" ? "Upload a ZIP with 3-credit SEM marksheet images." : "Upload multiple 3-credit SEM marksheet images."}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="flex items-center space-x-4 p-4 bg-muted/30 rounded-md">
                <div className="flex items-center space-x-2">
                  <input type="checkbox" id="auto-retry-sem3" checked={autoRetryEnabled} onChange={(e) => setAutoRetryEnabled(e.target.checked)} className="rounded"/>
                  <label htmlFor="auto-retry-sem3" className="text-sm font-medium">Auto-retry failed files</label>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {courseOutcomes.map((co) => (
                    <div key={co} className="border rounded-md p-3">
                      ...
                      <div className="grid grid-cols-5 gap-2"> {/* Adjusted for more Qs */}
                        ...
                        {Object.keys(questionGroups).filter((key) => !key.includes("_Total") && !key.match(/^Q\d+$/)).map((group) => ( // Q1-Q10 are handled above
      ...
      {selectedFile && (
        <Card className="w-full max-w-4xl mx-auto" id="marksheet-details-sem3">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Marksheet Details (SEM 3-Credits)</CardTitle>
      ...
      {selectedStatus && (
        <Card className="w-full max-w-4xl mx-auto" id="validation-issues-sem3">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Validation Issues (SEM 3-Credits)</CardTitle>
