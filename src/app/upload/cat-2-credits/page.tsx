...
// Define the question structure for CAT 2-Credits
...
// Maximum marks for each question and subpart for CAT 2-Credits
const maxMarks = {
  ...
  // Q6 subparts (each subpart can have max 20, but only one group should be active)
  "6a(i)": 20,
  "6a(ii)": 20,
  "6a(iii)": 20,
  "6b(i)": 20,
  ...
  "6c(ii)": 20,
  "6c(iii)": 20,
  Q6_Total: 20, // Only one group from 6A, 6B, or 6C should be active

  // Q7 subparts (each subpart can have max 20, but only one group should be active)
  "7a(i)": 20,
  "7a(ii)": 20,
  "7a(iii)": 20,
  "7b(i)": 20,
  ...
  "7c(ii)": 20,
  "7c(iii)": 20,
  Q7_Total: 20, // Only one group from 7A, 7B, or 7C should be active

  Grand_Total: 50, // Total of all the above (10 + 20 + 20 = 50)
}

// Group questions by their main number for CAT 2-Credits
...
// Define the Excel column structure for CAT 2-Credits
const excelColumns = [
  "Name",
  "RRN",
  "Course",
...
export default function UploadCat2CreditsPage() {
  ...
  const processExtractedMarks = (result: ExtractHandwrittenMarksOutput): ExtractHandwrittenMarksOutput => {
    ...
    // Track which main question numbers (6, 7) have parts detected
    ...
    result.marks.forEach((mark) => {
      ...
      if (match) {
        ...
        if (questionsByMainNumber[mainNum]) { // Check if mainNum is relevant for this marksheet type
    ...
    // For each detected main number (6, 7), enforce the rule that only one subgroup (a, b, c) should be present
  ...
  const autoRetryFailedFiles = async (results: ExtractHandwrittenMarksOutput[]) => {
    const currentStatuses = [...processingStatuses] // Use a snapshot
    const failedIndices = currentStatuses // Use the snapshot here
    ...
    for (const failedIndex of failedIndices) {
      const failedStatus = currentStatuses[failedIndex] // Use the snapshot here
  ...
  const calculateCOTotals = (data: ExtractHandwrittenMarksOutput, groupTotals: Record<string, number>) => {
    ...
    courseOutcomes.forEach((co) => {
      const total = courseOutcomeMapping[co].reduce((sum, question) => {
        if (question.startsWith("Q") && question.match(/^Q\d+$/) ) { // Simple Q like Q1, Q2.. Q5
          const qNum = question.substring(1)
          const mark = data.marks.find((m) => m.questionNumber === qNum)
          return sum + (mark ? Number.parseInt(mark.mark) || 0 : 0)
        } else { // Group questions like 6A, 7B
  ...
  const generateExcelData = useCallback(
    (data: ExtractHandwrittenMarksOutput[]) => {
      return data.map((item) => {
        ...
        // Add single digit questions (1-5 for CAT 2-credits)
        for (let i = 1; i <= 5; i++) {
          const qKey = `Q${i}`
          const mark = item.marks.find((m) => m.questionNumber === i.toString())
          rowData[qKey] = mark ? Number.parseInt(mark.mark) || 0 : 0
        }
        // Add group totals (6A, 6B, etc. and 7A, 7B, etc. for CAT 2-credits)
        Object.keys(questionsByMainNumber).forEach(mainNumKey => {
  ...
  const generateExcelFile = useCallback(
    (data: ExtractHandwrittenMarksOutput[]) => {
      if (data.length === 0) {
        setExcelFileBlob(null); // Ensure blob is cleared if no data
        return null;
      }
  ...
  const handleDownloadExcel = useCallback(() => {
    if (excelFileBlob) {
      FileSaver.saveAs(excelFileBlob, `cat_2_credits_marksheet_data_${new Date().toISOString().slice(0, 10)}.xlsx`)
      toast({
  ...
  const handleViewDetails = (data: ExtractHandwrittenMarksOutput) => {
    ...
    setTimeout(() => scrollToElement("marksheet-details-cat2"), 100)
  }

  const handleViewValidationIssues = (status: ProcessingStatus) => {
    setSelectedStatus(status)
    setTimeout(() => scrollToElement("validation-issues-cat2"), 100)
  }
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      <Card className="w-full max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            CAT Marksheet (2 Credits) - Batch Processing
          </CardTitle>
          <CardDescription>
            Upload 2-credit CAT marksheet images to extract and compile data.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs
          ...
          >
            ...
            <TabsContent value="single">
              <p className="text-sm text-muted-foreground mb-4">Upload a single 2-credit CAT marksheet image.</p>
            </TabsContent>
            <TabsContent value="multiple">
              <p className="text-sm text-muted-foreground mb-4">
                Upload multiple 2-credit CAT marksheet images.
              </p>
            </TabsContent>
            <TabsContent value="zip">
              <p className="text-sm text-muted-foreground mb-4">
                Upload a ZIP file containing 2-credit CAT marksheet images.
              </p>
            </TabsContent>
          </Tabs>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      {uploadMode === "single"
                        ? "Upload the 2-credit CAT marksheet image."
                        : uploadMode === "zip"
                          ? "Upload a ZIP archive containing 2-credit CAT marksheet images."
                          : "Upload multiple 2-credit CAT marksheet images."}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
              ...
              <div className="flex items-center space-x-4 p-4 bg-muted/30 rounded-md">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="rounded"
                  />
                  <label htmlFor="auto-retry-cat2" className="text-sm font-medium">
              ...
              <div className="border rounded-md p-4 space-y-4">
                    ...
                    <div key={co} className="border rounded-md p-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        {Object.keys(questionGroups)
                          .filter((key) => !key.includes("_Total") && !key.startsWith("Q"))
                          .map((group) => (
                            <button
                              key={group}
                              type="button"
                ...
                <p className="text-xs text-muted-foreground">
                  Select which questions contribute to each Course Outcome. Questions are mutually exclusive.
                </p>
              </div>
      ...
      {selectedFile && (
        <Card className="w-full max-w-4xl mx-auto" id="marksheet-details-cat2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Marksheet Details (CAT 2-Credits)</CardTitle>
              <CardDescription>
      ...
      {selectedStatus && (
        <Card className="w-full max-w-4xl mx-auto" id="validation-issues-cat2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Validation Issues (CAT 2-Credits)</CardTitle>
              <CardDescription>
